...
> Nota sobre datos volátiles: las versiones de K8s, el estado de APIs (Gateway API, Ingress) y qué controladores están vigentes cambian rápido (ej. ingress-nginx fue retirado en 2026, y Gateway API es la dirección recomendada). Verificá contra `kubernetes.io` al implementar.
...
El punto **actual y de criterio** (dato volátil, verificá): el ecosistema se está moviendo de Ingress hacia la **Gateway API**, el sucesor más expresivo y ahora estándar. Por qué el cambio: el Ingress quedó corto (poca expresividad, cada controller inventaba anotaciones propias no portables) y la Gateway API lo reemplaza con un modelo más rico, **orientado a roles** (separa la responsabilidad del operador de infraestructura de la del dev de la app) y más portable entre implementaciones. Además, controladores clásicos como **ingress-nginx fueron retirados/discontinuados** en 2026, lo que empuja a proyectos nuevos hacia Gateway API. La idea conceptual es la misma (un punto de entrada que rutea por host/path con TLS); cambia la API con la que lo declarás.
...
**Probes (chequeos de salud).** K8s sondea tus contenedores con HTTP/comandos para decidir qué hacer. Las dos clave —y confundirlas es un error común—:
...
**Autoscaling con el HPA (Horizontal Pod Autoscaler).** Acá se cierra el círculo del escalado: el HPA **ajusta automáticamente el número de réplicas** de un Deployment según una métrica (CPU, memoria, o métricas custom como requests/seg). "Si el uso de CPU promedio pasa el 70%, sumá réplicas (hasta 30); si baja, sacá (hasta 3)". Eso convierte el escalado manual del módulo 4 en automático y reactivo al tráfico —y, combinado con requests bien puestos, en eficiente en costo (el FinOps del módulo de AWS)—. (Hay también escalado vertical y de nodos, pero el HPA horizontal es el pan de cada día.)
