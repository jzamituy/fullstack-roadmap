...
> **Sidecars nativos (2026).** Históricamente un sidecar era "otro contenedor más" en el array `containers`, lo que traía dos problemas: no había garantía de que arrancara *antes* que tu app (race en el boot) ni de que se apagara *después* (perdías logs/conexiones del shutdown), y en un `Job` un sidecar que nunca termina dejaba el Pod colgado. Desde K8s **1.33 (2025)** el patrón moderno es declarar el sidecar como un **`initContainer` con `restartPolicy: Always`**: K8s garantiza que arranca antes del contenedor principal, sigue vivo durante toda la vida del Pod, y se termina después. Es lo que conviene usar hoy para proxies (service mesh) o recolectores de logs.
...
- Un **gestor de secretos externo** (AWS Secrets Manager, Vault) integrado, donde el secreto real vive afuera y K8s solo lo referencia. El patrón dominante hoy para esto es el **External Secrets Operator (ESO)**: un operador que sincroniza secretos desde el gestor externo hacia Secrets de K8s, así tu manifiesto (en git, GitOps) referencia el secreto sin que el valor real viva nunca en el repo.
...
**Qué puede hacer un Pod a nivel runtime: Pod Security Standards.** RBAC controla quién toca la *API*; otra capa controla qué privilegios pide un Pod *al correr* (¿root?, ¿host network?, ¿contenedor privilegiado?). La vieja **PodSecurityPolicy (PSP) fue eliminada en K8s 1.25**; su reemplazo son los **Pod Security Standards** (tres niveles: `privileged`, `baseline`, `restricted`), que se aplican **por namespace** vía el **Pod Security Admission** (una etiqueta en el namespace que rechaza o advierte Pods que no cumplen el nivel). Es pregunta de entrevista frecuente justamente porque mucha gente sigue nombrando la difunta PSP.
...
> **Antipatrón frecuente: liveness que chequea dependencias.** La liveness debe responder solo "¿mi proceso está vivo y no colgado?". Si tu endpoint de liveness además chequea la base, Redis o un servicio externo, una caída transitoria de *esa* dependencia hace que K8s **reinicie tus Pods en cascada** —empeorando justo cuando todo ya estaba mal—. El chequeo de dependencias va en la **readiness** (te saca del balanceo hasta que la dependencia vuelve, sin matarte). Regla: liveness = "el proceso responde"; readiness = "puedo atender de verdad ahora".
...
Eso convierte el escalado manual del módulo 4 en automático y reactivo al tráfico —y, combinado con requests bien puestos, en eficiente en costo (el FinOps del módulo de AWS)—. Para que el HPA pueda calcular el % de CPU necesita el **metrics-server** instalado, y el `requests` del contenedor como base del cálculo. Un **caveat** que conviene saber: escalar por **memoria** suele ser mala señal (muchos runtimes no devuelven memoria tras el GC, así que la métrica no baja y no hay scale-down); CPU o métricas custom (req/seg, vía un adapter) son lo habitual.

**Dos escalados distintos (no los confundas).** El HPA agrega **más Pods** —pero si los nodos ya están llenos, esos Pods quedan en `Pending` y no arranca nada—. Agregar **más nodos** (máquinas) es trabajo del **Cluster Autoscaler** o, más moderno, **Karpenter** (en AWS): ven los Pods `Pending` y aprovisionan nodos para ellos, y los retiran cuando sobran. Son las dos mitades: HPA escala la *app*, Cluster Autoscaler/Karpenter escala la *infraestructura*. Esto conecta directo con el costo del módulo 11: el HPA por sí solo no te ahorra plata si dejás 10 nodos encendidos al pedo; recién con autoscaling de nodos el cluster sigue al tráfico de verdad (y aun así nunca llega al *scale-to-zero* de Fargate/serverless).
...
Acá se cierra un hueco que dejó el módulo 5: un Service normal te da **una** IP estable que balancea entre Pods intercambiables, pero un StatefulSet necesita justo lo contrario —llegar a *una réplica puntual*—. Para eso se usa un **headless Service** (`clusterIP: None`): en vez de una IP virtual, el DNS devuelve la dirección de **cada Pod individualmente**, con un nombre estable por réplica (`db-0.mi-svc`, `db-1.mi-svc`). Es lo que hace que la réplica primaria de una base sea direccionable de forma fija. Por eso el StatefulSet siempre va de la mano de un headless Service.
...
`kubectl logs`/`describe` sirven para **un Pod, en el momento, de forma reactiva** —bien para apagar un incendio puntual—. Pero operar un cluster de verdad pide la vista **agregada e histórica**: cuántos Pods reinician en el tiempo, latencia y errores por servicio, saturación de nodos. Eso ya no es `kubectl`: es **observabilidad** —`kube-state-metrics` y el `metrics-server` exponen el estado del cluster, Prometheus lo recolecta, Grafana lo grafica, y el tracing/OpenTelemetry cubre las requests—. Es exactamente el siguiente módulo del track; `kubectl logs` es el puente, no el destino.
...
11.5 **Decisión escrita (caso real).** Sos el Tech Lead de un equipo de 4 personas con una API Node, un worker de colas y un frontend, todo en un solo cloud, ~3 deploys por semana. El CTO propone migrar a Kubernetes "porque es el estándar". Escribí 4-6 líneas justificando tu recomendación: ¿K8s o una alternativa gestionada? Nombrá los factores concretos que pesan y qué te haría cambiar de opinión.
...
11.5 Recomendación esperable: NO migrar a K8s. Tres servicios, un solo cloud, equipo de 4 y baja
     frecuencia de deploy → la carga operativa de un cluster (upgrades, networking, RBAC, nodos,
     on-call) no se paga con el beneficio. Alternativa: contenedores gestionados (Fargate/ECS,
     Cloud Run, App Runner) o un PaaS; escalan, operan y cuestan por uso, y el equipo sigue
     enfocado en producto. Factores que pesan: cantidad de servicios, tamaño/experiencia del
     equipo, necesidad de portabilidad multi-cloud, requisitos de control fino. "Es el estándar"
     no es un factor técnico. Qué cambiaría la decisión: crecer a decenas de servicios con
     orquestación compleja, una exigencia real de portabilidad/on-prem, o contar con una
     plataforma/SRE dedicada que absorba la operación.
```
