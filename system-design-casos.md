...
> **Una respuesta Staff narra una evolución, no un monumento.** El error de presentación más común es saltar al diseño final y defenderlo como si siempre hubiera sido obvio. La jugada senior es contar el **arco**: **v1** = la versión más simple que cumple los requisitos (a baja escala suele alcanzar — es el "no sobre-diseñar" de cada cierre); **v2** = el primer cuello que aparece al crecer y el cambio puntual que lo resuelve; **v3** = el diseño a plena escala con sus trade-offs. Mostrar *qué cuello concreto* dispara cada salto —y recién ahí subir la complejidad— demuestra criterio mucho mejor que dibujar el monumento de entrada. En los cierres de los **casos 1 y 8** vas a ver ese arco explícito; en los otros, el cierre se queda en el v1/v2 que **basta** —porque la otra mitad del criterio Staff es saber cuándo **no** escalar—, y el arco completo solo aparece donde el problema lo pide. Entrenate a contar cualquier caso así.
...
> **El cierre de criterio — la evolución v1→v2→v3.** No saltes al híbrido: contá cómo se llega. **v1 (miles de usuarios):** fan-out **on read** — `WHERE authorId IN (seguidos) ORDER BY createdAt LIMIT 20` sobre Postgres con un índice. A miles, el JOIN es trivial y precomputar sería infra ociosa; alcanza y sobra. **v2 (el feed se siente lento al crecer):** el costo de rearmar el timeline crece con la cantidad de seguidos y se paga en **cada** apertura → movés el costo a la escritura con fan-out **on write** (precomputás el timeline en Redis, leer pasa a ser un **rango ya materializado**, sin recalcular el join). Lo pagás con **escrituras amplificadas y datos algo stale**. **v3 (grafo desbalanceado, celebridades):** el push puro se rompe con los 100M-seguidores → **híbrido** (push para normales + pull para celebridades + merge en lectura), al costo de **dos caminos + merge**. Cada salto lo dispara un cuello **concreto**, no la ambición: ese arco es la respuesta, no el dibujo final.
...
> **El cierre de criterio — la evolución v1→v2→v3.** **v1 (stock amplio, baja contención):** un `UPDATE productos SET stock = stock - 1 WHERE id = $1 AND stock > 0` atómico, chequeando filas afectadas. Sin holds, sin saga, sin sala de espera — alcanza y sobra para un e-commerce común. **v2 (recurso escaso + el pago externo tarda):** tres mecanismos, cada uno a su cuello — el **hold con TTL** libera el stock si el pago no llega; la **idempotency key** evita el doble cobro ante reintentos; las **tres fases** (reserva → pago → confirmación) separan el hold del commit para no tener la transacción abierta esperando al proveedor. **v3 (pico masivo, contención brutal sobre pocas filas):** el TTL libera por expiración **pasiva**, pero con varios pasos acoplados y compensaciones que coordinar necesitás compensación **activa y orquestada** → **saga** + **sala de espera virtual** que admite de a tandas, al costo de complejidad operativa. La maquinaria completa es de v3; saber cuándo *no* la necesitás es la otra mitad del criterio.
