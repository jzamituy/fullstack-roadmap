...
**Lo que asumimos.** TS, NestJS (DI, providers), el tool use del módulo de LLMs, el RAG del módulo anterior, async/await, manejo de errores, colas y resiliencia. El código de los ejemplos usa el **SDK core** `@anthropic-ai/sdk` (el **Claude Agent SDK** es un paquete aparte; lo vemos en el módulo 10).
...
```ts
const MAX_ITERS = 10; // el guardarraíl del módulo 8: ni loop infinito ni quemar tokens
...
async function correrAgente(objetivo: string, tools: Anthropic.Tool[]): Promise<string> {
  ...
  for (let i = 0; i < MAX_ITERS; i++) {
    ...
    // Chequeá stop_reason ANTES de leer content (como en el módulo de LLMs):
    if (res.stop_reason === "refusal") {
      throw new Error("El modelo se rehusó a continuar");
    }
    if (res.stop_reason === "max_tokens") {
      throw new Error("Respuesta truncada: subí max_tokens o acortá el contexto");
    }
    if (res.stop_reason === "pause_turn") {
      // Tool del lado del servidor: reenviá lo que va, SIN agregar un mensaje "continuá"
    ...
    // stop_reason === "tool_use": ejecutar cada tool y devolver resultados
  ...
  throw new Error(`El agente no terminó en ${MAX_ITERS} iteraciones`);
}
```
...
- **Loops infinitos y costo**: cada vuelta es una llamada (input + output) sobre un historial cada vez más largo. Sin límites, un agente puede no terminar nunca o quemar tokens. Por eso el loop de arriba usa un **tope de iteraciones** (un `for` con máximo, no un `while` infinito en producción) — el guardarraíl del módulo 8.

Dos detalles de la API real: para **tools del lado del servidor** (web search, code execution) podés recibir `stop_reason === "pause_turn"` —el modelo pausó y querés reenviar para que siga, sin agregar un mensaje "continuá"—. Y el **SDK core** (`@anthropic-ai/sdk`) trae un **tool runner** (`client.beta.messages.toolRunner`) que maneja este loop por vos —no lo confundas con el **Claude Agent SDK** del módulo 10, que es un paquete aparte y mucho más grande—; entenderlo a mano —como acá— es clave para saber qué hace por debajo y cuándo necesitás el control manual (gating, human-in-the-loop).
...
**Teoría.** Un agente que corre muchas vueltas tiene un problema que un workflow corto no tiene: **el contexto crece sin parar.** Cada tool result, cada paso de razonamiento, se acumula en `messages` (módulo 3). Y aunque Opus 4.8 tenga una ventana de **1M tokens**, el problema dominante en un agente largo no suele ser chocar literalmente esa pared, sino dos costos que aparecen mucho antes: el **costo por token** de reenviar un historial cada vez más grande en cada vuelta, y la **degradación de calidad** (el fenómeno *lost in the middle* del módulo de LLMs: el modelo rinde peor cuanto más enorme es el contexto). Manejar esto es lo que separa un agente de demo de uno que corre tareas largas.
...
Un detalle que conecta con el caché: cambiar el system prompt o las tools a mitad de sesión **invalida el prompt caching** (módulo 9 de LLMs: la caché es prefix-match). Por eso los agentes serios evitan editar el prefijo estable; si necesitan inyectar instrucciones a mitad de camino, lo hacen *después* del prefijo cacheado —como un mensaje más del historial (un turno de `user` con la instrucción nueva), no editando el bloque `system` estable—. El criterio de "lo estable primero" del módulo de caché vuelve, ahora en el diseño del agente.
...
- **Vos hospedás el loop.** *Tu* infraestructura corre el loop y ejecuta las herramientas. Acá hay dos niveles de andamiaje, según cuánto quieras armar a mano —y son **dos cosas distintas**, no las confundas—:
  - **El `tool runner` del SDK core** (`@anthropic-ai/sdk`, `client.beta.messages.toolRunner`): automatiza el loop del módulo 3 —pedir tools, ejecutarlas, reinyectar resultados— y nada más. Todo lo demás (gates, contexto, logging) lo ponés vos. Ideal cuando querés el loop resuelto pero el resto bajo tu control.
  - **El Claude Agent SDK** (paquete aparte: `@anthropic-ai/claude-agent-sdk`): un harness de producción **completo** —loop + herramientas built-in (leer/escribir archivos, bash, grep…) + gestión de contexto y compaction + subagentes + hooks + cliente MCP integrado—. Es lo que querés cuando vas a construir un agente serio y no querés reimplementar todo el andamiaje a mano.

  En ambos casos lo elegís cuando querés control total e integrarlo en tu backend NestJS como un service más. Es el camino por defecto para un backend que ya tenés.
...
11.2 (a) Llamada simple: clasificar en 3 categorías es un solo paso especificable. (Ojo: si
     además usaras esa clasificación para mandar cada email a un flujo distinto, eso ya sería
     *routing*, que es un workflow del módulo 5 —no una llamada—.) (b) Agente: tarea abierta,
