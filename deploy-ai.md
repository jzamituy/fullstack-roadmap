...
Las herramientas de facto: **llama.cpp + GGUF** (el estándar para hardware de consumo: CPU, GPU de consumo, Apple Silicon), con envoltorios fáciles como **Ollama** y **LM Studio**. Distinción de formatos respecto al módulo 4: **GGUF** (cuantización PTQ, CPU/mixto, portable, para local) vs **AWQ/GPTQ** (orientados a GPU de servidor, para vLLM). Regla práctica de cuantización por VRAM disponible (para modelos del orden de 7–8B): <8 GB → Q3/Q2 (**último recurso: Q2/Q3 degradan la calidad de forma apreciable, no son equivalentes a Q4_K_M**); **8–12 GB → Q4_K_M (el recomendado, mejor relación calidad/tamaño)**; 16–24 GB → Q8; 24 GB+ → fp16 (ojo: fp16 de un 8B son ~16 GB de pesos, así que en 24 GB queda poco margen para el KV cache en contextos largos). Modelos típicos: familias Llama, Gemma, Phi, Qwen en 1B–8B (muchos producidos por **distillation** de un modelo grande — puente con [fine-tuning.md](fine-tuning.md)).
